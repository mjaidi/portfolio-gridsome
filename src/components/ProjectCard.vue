...
export default {
  ...
  computed: {
    style() {
      if (this.hovering) {
        return `background-image: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url(${this.project.coverImage.src})`;
      } else {
        return `background-image: url(${this.project.coverImage.src})`;
      }
    }
  },
  methods: {
